...
function createError(reason, err) {
    return `Unable to load data due to failure to ${reason}: ${err}`;
}

function populateTypeOfData(opts, data) {
    if (opts.contentType === 'CSV') {
        opts.file = data;
        info('content type determined to be a CSV file');
    } else {
        opts.data = data;
        info('content type determined to be JS array');
    }
}
...
function processJob(opts, data, cb) {
    ...
    batchSplitter.split({
        ...
        if (err) return cb(createError('split job into multiple batches', err));
        ...
        bulkApi.createJob({
            ...
            if (err) return cb(createError('create job', err));
            ...
            async.map(batches, (batch, callback) => {
                ...
                if (err) {
                    err = createError('process batch', err);
                }
